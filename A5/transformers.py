...
def prepocess_input_sequence(
    ...
    for i in range(len(input_str_split)):
        in_dict =  input_str_split[i] in token_dict
        if(not in_dict):
            input_str_split_number = input_str_split[i]
            for j in range(len(input_str_split_number)):
                out.append(token_dict[input_str_split_number[j]])
        else:
            out.append(token_dict[input_str_split[i]])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    (K,M) = key.shape
    M = torch.tensor((M),dtype=key.dtype,device=key.device)
    out = torch.zeros_like(key)
    E = (query @ key.T) / torch.sqrt(M)
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    attn_scores = torch.matmul(query, key.transpose(-2, -1))  # Shape: (N, K, K)
    ...
    if mask is not None:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        attn_scores = attn_scores.masked_fill(mask=mask,value=float('-inf'))
    # Replace "pass" statement with your code
    # Apply softmax to get attention weights
    weights_softmax = F.softmax(attn_scores, dim=-1)  # Shape: (N, K, K)

    # Compute the weighted sum of values
    #print("good")
    y = torch.matmul(weights_softmax, value)  # Shape: (N, K, M)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        query_q = self.q(query)
        key_k = self.k(key)
        value_v = self.v(value)
        y,weights_softmax = scaled_dot_product_no_loop_batch(query_q,key_k,value_v,mask)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        #print(f"num_heads:{num_heads},dim_in:{dim_in},dim_out:{dim_out}, dim_in_infer:{dim_out * num_heads}")
        self.singleHeadattention = torch.nn.ModuleList([SelfAttention(dim_in=dim_in,dim_q=dim_out ,dim_v=dim_out) for i in range(num_heads)])
        self.output_projection = nn.Linear(dim_out*num_heads, dim_in)
        torch.nn.init.xavier_normal_(self.output_projection.weight)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        heads = [singleattentinon(query,key,value,mask) for singleattentinon in self.singleHeadattention]
        
        multihead_output = torch.cat(heads, dim=-1)
        
        y = self.output_projection(multihead_output)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        deviation = ((x-mean)**2).mean(dim=-1,keepdim=True)

        std = torch.sqrt(deviation + self.epsilon)
        
        nomliziation = (x-mean) / std
        
        y = self.scale * nomliziation +self.shift
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        self.mlp_first = nn.Linear(inp_dim,hidden_dim_feedforward)
        self.relu = nn.ReLU()
        self.mlp_seconde = nn.Linear(hidden_dim_feedforward,inp_dim)
        torch.nn.init.xavier_normal_(self.mlp_first.weight)
        torch.nn.init.xavier_normal_(self.mlp_seconde.weight)
    ...
    def forward(self, x):
        ...
        # Replace "pass" statement with your code
        out = x.clone()
        out1 = self.mlp_first(out)
        out2 = self.relu(out1)
        out3 = self.mlp_seconde(out2)
        y = out3
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.Mutiheads = MultiHeadAttention(num_heads=num_heads, dim_in=emb_dim, dim_out= emb_dim // num_heads)
        #print(num_heads*emb_dim)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.Mutiheads(x, x, x)  # Q = K = V = x
        # 2. Residual connection after MultiHead Attention + LayerNorm + Dropout
        out2 = self.Layernorm1(x + out1)  # Residual connection and LayerNorm
        out2 = self.dropout(out2)  # Apply Dropout

        # 3. FeedForward block
        out3 = self.feedforward(out2)

        # 4. Residual connection after FeedForward + LayerNorm + Dropout
        out = self.Layernorm2(out3 + out2)  # Residual connection and LayerNorm
        out_ = self.dropout(out) # Apply Dropout to final output
        """
        #print("Before LayerNorm1:", out1 + x)
        #print("After LayerNorm1:", out2)
        print("After FeedForward:", out3)
        """
        y = out_
        ##########################################################################
        #               END OF YOUR CODE                                         #
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    (N,K) = seq.shape
    mask = torch.triu(torch.ones((N,K,K),device=seq.device,dtype=seq.dtype),diagonal=1)
    mask = mask == 1
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.attention_self.forward(dec_inp,dec_inp,dec_inp,mask=mask)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # Step 1: Create a 1D tensor of length K where nth element is n/K
    positions = torch.arange(K).float() / K  # tensor of shape (K,)
    # Step 2: Replicate this tensor M times to create a tensor of shape (1, K, M)
    y = positions.unsqueeze(1).repeat(1, 1, M)  # Shape (1, K, M)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    a = torch.arange(M) // M
    divs = torch.pow(10000,-1*a).unsqueeze(0)
    out[ : ,0::2] = (torch.sin(position * divs))[:,0::2]
    out[:,1::2] = (torch.cos(position * divs))[:,1::2]
    y = out.reshape(1,K,M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        (N,K,M) = a_emb_inp.shape 
        mask = get_subsequent_mask(torch.zeros((N,K),device=a_emb_inp.device))
