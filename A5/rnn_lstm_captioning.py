...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    Wx_local = Wx.clone()
    b_local = b.clone()
    prev_h_local = prev_h.clone()
    Wh_local = Wh.clone() 
    a = x_local @ Wx_local + b_local + prev_h_local @ Wh_local
    next_h =  torch.tanh(a)
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    da = 2 * dnext_h / (torch.ones_like(dnext_h) + torch.cosh(2*a))  
    dx = da @ Wx.T
    dprev_h = da @ Wh.T
    dWh = prev_h.T @ da
    dWx = x.T @ da
    db = da.T @ torch.ones((da.shape[0],),device=da.device,dtype=da.dtype)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if(i < 1):
            h[:,i,:],cache_ = rnn_step_forward(x[:,i,:],h0,Wx=Wx,Wh=Wh,b=b)
            cache.append(cache_)
        else:
            h[:,i,:],cache__ = rnn_step_forward(x[:,i,:],h[:,i-1,:],Wx=Wx,Wh=Wh,b=b)
            cache.append(cache__)
    cache.append(x)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_backward(dh, cache):
    ...
    # Replace "pass" statement with your code
    (N,T,H) = dh.shape
    dWx_s ,dWh_s ,db_ = None,None,None
    x = cache[T]
    D = x.shape[2]
...
# Iterate backward over the sequence
    for t in range(T - 1, -1, -1):
        dh_current = dh[:, t, :] + dprev_h  # Include gradient from future time step
        dx[:, t, :], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_current, cache[t])
        dWx =  dWx + dWx_t
        dWh =  dWh + dWh_t
        db = db + db_t

# Set the initial hidden state gradient
        dh0 = dprev_h
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    N, T, V = x.shape
    x_flat = x.view(N * T, V)  # Reshape to (N*T, V)
    y_flat = y.reshape(N * T)  # Reshape to (N*T)

    loss = torch.nn.functional.cross_entropy(x_flat,y_flat,ignore_index= ignore_index,reduction="sum") / N
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.imgae_encoder = ImageEncoder(pretrained=image_encoder_pretrained, verbose=False)
        ...
        if(cell_type == "rnn"):
            ...
            if(cell_type == "lstm"):
                ...
                self.feature_projection = nn.Conv2d(in_channels=input_dim, out_channels=hidden_dim, kernel_size=1)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        # Step 1: Extract image features using the CNN backbone
        
        features = self.imgae_encoder.forward(images)  # Shape: (N, D)
        N = images.shape[0]
        D = features.shape[1]
        
        # Step 2: Project image features to the initial hidden state (h0) or attention input (A)
        if self.cell_type == 'attn':
            A = self.feature_projection(features)# Shape: (N, H, 4, 4)
        else:
            h0 = self.feature_projection(features.mean(dim=(2, 3)))  # Shape: (N, H)
        
        # Step 3: Transform input captions to word embeddings
        word_embeddings = self.word_embedding(captions_in)  # Shape: (N, T, W)
        # Step 4: Process the word embeddings with RNN or LSTM
        if self.cell_type == 'attn':
            hidden_states = self.attn(word_embeddings, A)  # Shape: (N, T, H)
        else:
            if self.cell_type == "rnn":
                hidden_states = self.rnn(word_embeddings, h0)  # Shape: (N, T, H)
            else:
                hidden_states = self.lstm(word_embeddings,h0)
        # Step 5: Compute scores over the vocabulary
        vocab_scores = self.output_projection(hidden_states)  # Shape: (N, T, V)

        # Step 6: Compute the temporal softmax loss
    ...
    def sample(self, images, max_length=15):
        ...
        #######################################################################
        # Replace "pass" statement with your code
        features = self.imgae_encoder(images)
        if(self.cell_type == "attn"):
            ...
            c0 = torch.zeros_like(h0) if self.cell_type == "lstm" else None

        current_word = images.new(N).fill_(self._start).long()
        print(current_word.shape)
        for t in range(max_length):
        # Embed the current word
                word_embed = self.word_embedding(current_word)  # Shape: (N, W)
                                                                 # RNN step
                if self.cell_type == "rnn":
                    h0 = self.rnn.step_forward(word_embed, h0)  # Shape: (N, H)
                elif self.cell_type == "lstm":
                    h0, c0 = self.lstm.step_forward(word_embed, h0, c0)  # Shape: (N, H), (N, H)
                elif self.cell_type == "attn":
                    attn,attn_weights = dot_product_attention(h0,A)
                    h0, c0 = self.attn.step_forward(word_embed, h0, c0,attn)  # Shape: (N, H), (N, H), (N, 4, 4)
                    attn_weights_all[:, t] = attn_weights

                # Compute scores for the vocabulary
                scores = self.output_projection(h0)  # Shape: (N, V)

                # Select the word with the highest score
                current_word = scores.argmax(dim=1)  # Shape: (N,)

                # Store the current word in captions
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        i_gate = torch.sigmoid(A[:N,:H])
        f_gate = torch.sigmoid(A[:N,H:2*H])
        o_gate = torch.sigmoid(A[:N,2*H:3*H])
        g_gate = torch.tanh(A[:N,3*H:4*H])
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(T):
            if i == 0:
                new_h, prev_c = self.step_forward(x[:, i, :], h0, prev_c=c0)
            else:
                new_h, prev_c = self.step_forward(x[:, i, :], h[:, i-1, :].clone(), prev_c=prev_c)
            
            h[:, i, :] = new_h.clone()  # 避免 in-place 修改计算图
...
def dot_product_attention(prev_h, A):
    ...
    A_flat = A.view(N,H,-1)
    prev_h = prev_h.clone()
    attn_scores = torch.bmm(prev_h.unsqueeze(1), A_flat)  # (batch, 1, 16)
    attn_scores = attn_scores.squeeze(1) / (H ** 0.5)  # Scale by sqrt(H), shape: (batch, 16)
    
    attn_weights = torch.softmax(attn_scores,dim=1)
    
    attn = torch.bmm(A_flat, attn_weights.unsqueeze(2)).squeeze(2)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for i in range(T):
            if i == 0:
                attn,attn_weights = dot_product_attention(h0,A)  
                new_h, prev_c = self.step_forward(x[:, i, :], h0, prev_c=c0,attn=attn)
            else:
                attn,attn_weights = dot_product_attention(h[:, i-1, :],A=A)
                new_h, prev_c = self.step_forward(x[:, i, :], h[:, i-1, :].clone(), prev_c=prev_c,attn=attn)
