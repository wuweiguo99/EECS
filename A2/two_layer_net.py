...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    h_get = X @ W1 + b1
    hidden = torch.maximum(torch.zeros_like(h_get),h_get)
    scores = hidden @ W2 + b2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    scores -= max_scores
    exp_div = torch.sum(torch.exp(scores),dim=1,keepdim=True)
    exp = torch.sum(torch.exp(scores),dim=1,keepdim=True)
    exp_dm = torch.exp(scores)
    svm_scores = (exp_dm / exp_div)[torch.arange(N),y]
    loss -= torch.sum(torch.log(svm_scores)) / N   
    ...
    # Replace "pass" statement with your code
    dh_w1 = X.T #(D,N)
    dhi_h = torch.zeros_like(h1)
    dhi_h[h1 > 0] = 1 #(N,H)
    dhi_w1 = dh_w1 @ dhi_h 
    dsocers_hl = W2.t() #(C,H)
    dsocers_W2 = h1.t() #(H,N)
    
    indices = torch.stack([torch.arange(N,dtype=X.dtype,device=X.device), y], dim=0)
    values = torch.ones(N, dtype=X.dtype,device=X.device)
    Y_shpase = torch.sparse_coo_tensor(indices, values, (N, scores.shape[1]))
    exp_div = torch.exp(scores)
    exp_dm =torch.sum(torch.exp(scores),dim=1).unsqueeze(1)
    exp = exp_div / exp_dm
    dl_scoers = exp - Y_shpase #(N,C)
    dl_hi = torch.mm(dl_scoers,dsocers_hl)
    dl_hi[h1 <= 0] = 0
    
    dW2 = (torch.sparse.mm(dsocers_W2,dl_scoers)) / N
    dW1 = torch.mm(dh_w1,dl_hi) / N
    dl_w2 = dW2 + 2*reg * W2
    dl_w1 = dW1 + 2*reg * W1
    grads['W2'] = dl_w2
    grads["W1"] = dl_w1
...
def nn_predict(
    ...
    # Replace "pass" statement with your code
    y_pred = 0
    y_span = loss_func(params, X, y=None)
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.4e0,1.45e0]
    hidden_sizes = [128,256,512]
    regularization_strengths = [8e-4]
    learning_rate_decays = [0.94,0.95]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    hyperparamtuple = get_param_set_fn()
    stats_dict = {}
    
    for lr in hyperparamtuple[0]:
        for hs in hyperparamtuple[1]:
            for reg in hyperparamtuple[2]:
                for lrd in hyperparamtuple[3]:
                    ...
                    current_val_acc = 100.0 * (y_val_pred == data_dict['y_val']).double().mean().item()
                    if current_val_acc > best_val_acc:
                        best_net = net
                        best_stat = stats
                        best_val_acc = current_val_acc
                    print('learning rate: {},hidden size : {},regs:{},decay :{}'.format(lr,hs,reg,lrd),",val_acc: %.2f%%" % current_val_acc)
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
