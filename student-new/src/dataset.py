...
class CharCorruptionDataset(Dataset):
    ...
    def __len__(self):
        # returns the length of the datase
    ...
    def __getitem__(self, idx):
        ...
        # Step 1: Randomly truncate the document
            ...
            truncated_length = random.randint(min_length, min(max_length, len(document)))
            truncated_doc = document[:truncated_length]
                
                # Step 2: Break the truncated document into prefix, masked_content, suffix
            mask_length = max(1, int(len(truncated_doc) / 4 * (random.uniform(0.5, 1.5))))
            mask_start = random.randint(0, len(truncated_doc) - mask_length)
            ...
            masked_string = (
                ...
                # Pad the string to block_size
            masked_string = masked_string.ljust(self.block_size, self.PAD_CHAR)
                
                # Step 4: Create input-output pair
            x_str = masked_string[:-1]
            y_str = masked_string[1:]
                
                # Step 5: Encode to Long tensors
