...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        x_c = x.clone().view(num_train,-1) # (N,D)
        dw = (x_c.T) @ dout
        dx =  torch.reshape((dout @ (w.T)),size)
        #dx = torch.zeros_like(x)
        db = dout.T @ (torch.ones((num_train,),device=b.device,dtype=b.dtype))
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += (self.reg*torch.sum(self.params["W2"]*self.params["W2"])) + self.reg*torch.sum(self.params["W1"]*self.params["W1"])
        dh,grads["W2"],grads["b2"] = Linear.backward(ds,cache_2)
        #grads["W2"] /= X.shape[0]
        grads['W2'] += 2*self.reg*self.params["W2"]
        dx,grads["W1"],grads["b1"] = Linear_ReLU.backward(dh,cache=cache)
       #grads['W1'] /= X.shape[0]
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        i = 1
        input_di = input_dim
        for hi in hidden_dims:
          name_layer_w = "W" + str(i)
          name_layer_b = "b" + str(i)
          self.params[name_layer_w] = weight_scale * torch.randn(
            input_di, hi, dtype=dtype, device=device)
          self.params[name_layer_b] = torch.zeros(hi, dtype=dtype, device=device)
          input_di = hi
          i+= 1
        self.params["W" + str(i)] = weight_scale * torch.randn(
            input_di, num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        h_get = []
        cache = []
        out,cache_ = Linear_ReLU.forward(X,self.params["W1"],self.params["b1"])
        h_get.append(out)
        cache.append(cache_)
        for i in range(1,self.num_layers-1):
          out,cache_ = Linear_ReLU.forward(h_get[i - 1],self.params["W" + str(i + 1)],self.params["b" +str(i + 1) ])
          h_get.append(out)
          cache.append(cache_)
        out,cache_ = Linear.forward(h_get[i],self.params["W" + str(i + 2)],self.params["b" +str(i + 2) ])
        h_get.append(out)
        cache.append(cache_)
        scores = h_get[i+1]
        #################################################################
        ...
        for j in range(1,self.num_layers + 1):
          ...
          #print(self.params["W"+str(j)].shape)
        dout = dl
        dx,grads["W" + str(self.num_layers)],grads["b" + str(self.num_layers)] = Linear.backward(dout,cache=cache[self.num_layers - 1])
        grads["W" + str(self.num_layers)] += 2*self.reg*self.params["W" + str(self.num_layers)]
        dout = dx
        for j in range(self.num_layers-2,-1,-1):
          dx,grads["W" + str(j + 1)],grads["b" + str(j + 1)] = Linear_ReLU.backward(dout,cache[j])
          grads["W" + str(j + 1)] += 2 * self.reg * self.params["W" + str(j + 1)]
          dout = dx
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # Replace "pass" statement with your code
    lr = 1.3e-1
    solver = Solver(model=model,data=data_dict,optim_config={'learning_rate': lr,},lr_decay=0.97,print_every=1000,device = device)
    solver.batch_size = 150
...
def get_five_layer_network_params():
    ...
    # Replace "pass" statement with your code
    learning_rate = 2  # Experiment with this!
    weight_scale = 1e-3
    ################################################################
    #                       END OF YOUR CODE                       #
    ################################################################
...
def sgd_momentum(w, dw, config=None):
    ...
    w -= config['momentum'] * v - (1 + config['momentum']) * v
    next_w = w
    ###################################################################
    #                           END OF YOUR CODE                      #
    ###################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = torch.randn(x.shape,device=x.device,dtype=x.dtype) < p
            out = x*mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
