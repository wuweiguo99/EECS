...
import fully_connected_networks
from fully_connected_networks import Linear_ReLU, Linear, Solver, adam, ReLU
import importlib
importlib.reload(fully_connected_networks)
from fully_connected_networks import Linear_ReLU, Linear, Solver, adam, ReLU
...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        F,_,HH,WW = w.shape[0],w.shape[1],w.shape[2],w.shape[3]
        stride = conv_param['stride']
        H_ = (int) (1 + (H + 2 * conv_param['pad'] - HH) / conv_param['stride'])
        W_ = (int) (1 + (W + 2 * conv_param['pad'] - WW) / conv_param['stride'])
        x_c = torch.clone(x)
        x_c = torch.nn.functional.pad(x_c,pad=(conv_param['pad'],conv_param['pad'],conv_param['pad'],conv_param['pad']))
        out = torch.zeros((N,F,H_,W_),device=w.device,dtype=x.dtype)
        for i in range(N):  
          for j in range(F):
            for k in range(H_):
              for d in range(W_):
                #out[i,j,k,d] = ((x_c[i,0:,0+k*stride:HH+k*stride,0+d*stride:WW+d*stride]).flatten()).dot((w[j,0:,0:,0:]).flatten()) + b[j]
                out[i,j,k,d] = torch.sum((x_c[i,0:,0+k*stride:HH+k*stride,0+d*stride:WW+d*stride])*(w[j,0:,0:,0:])) + b[j]
    ...
    # Dimensions
        ...
        xp = torch.nn.functional.pad(x, (pad, pad, pad, pad), mode='constant', value=0)
        ...
        doutp = torch.nn.functional.pad(dout, (WW-1, WW-1, HH-1, HH-1), mode='constant')

        # 0-padding juste sur les deux dernières dimensions de dx
        dxp = torch.nn.functional.pad(x, (pad, pad, pad, pad), mode='constant', value=0)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.params["W1"] = weight_scale*torch.randn((num_filters,input_dims[0],filter_size,filter_size),device=device,dtype=dtype)
        self.params["b1"] = torch.zeros(num_filters, dtype=dtype, device=device)
        input_dim =(int) (num_filters*input_dims[1]*input_dims[2]/4)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        #c_get,cache_1 = FastConv.forward(X,self.params["W1"],self.params["b1"],conv_param=conv_param)
        #p_get,cache_2 = FastMaxPool.forward(c_get,pool_param=pool_param)
        p_get,cache_1 = Conv_ReLU_Pool.forward(X,self.params["W1"],self.params["b1"],conv_param=conv_param,pool_param=pool_param)
        h_get,cache_3 = Linear_ReLU.forward(p_get,self.params["W2"],self.params['b2'])
        scores,cache_4 = Linear.forward(h_get,self.params['W3'],self.params['b3'])
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        for j in range(1,4):
          loss += self.reg*torch.sum(self.params["W"+str(j)]*self.params["W" + str(j)])
        dh,grads["W3"],grads["b3"] = Linear.backward(ds,cache_4)
        grads['W3'] += 2*self.reg*self.params["W3"]
        dp,grads["W2"],grads['b2'] = Linear_ReLU.backward(dh,cache_3)
        grads['W2'] += 2*self.reg*self.params["W2"]
        dx,grads['W1'],grads['b1'] = Conv_ReLU_Pool.backward(dp,cache_1)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for ly in range(1,self.num_layers):
          if weight_initializer == None:
            self.params["W" + str(ly)] = weight_scale*torch.randn((num_filters[ly - 1],channel,3,3),device=device,dtype=dtype)
            self.params["b" + str(ly)] = torch.zeros((num_filters[ly -1],),device=device,dtype=dtype)
          self.params["W" + str(ly)] = kaiming_initializer(num_filters[ly - 1],channel,3,device=device,dtype=dtype)
          self.params["b" + str(ly)] = torch.zeros((num_filters[ly -1],),device=device,dtype=dtype)
          channel = num_filters[ly - 1]
        input_dim = input_dims[1] * input_dims[2]
        input_dim /= pow(2,2*len(max_pools))
        input_dim *= num_filters[ly - 1]
        #self.params["W" + str(ly + 1)] = weight_scale*torch.randn(((int)(input_dim),num_classes),device=device,dtype=dtype)
        self.params["W" + str(ly + 1)] = kaiming_initializer((int)(input_dim),num_classes,K=None,device=device,dtype=dtype)
        self.params["b" + str(ly + 1)] = torch.zeros((num_classes,),device=device,dtype=dtype)
        self.batchnorm = batchnorm
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        x = X
        for ly in range(1,self.num_layers):
          if ly - 1 in self.max_pools:
            out,cache_ = Conv_ReLU_Pool.forward(x,self.params["W" + str(ly)],self.params["b" + str(ly)],conv_param=conv_param,pool_param=pool_param)
            h_get.append(out)
            cache.append(cache_)
            x = h_get[ly - 1]
            if self.batchnorm:
              pass
              #h_get[ly - 1],cache[ly -1] = Conv_BatchNorm_ReLU_Pool.forward()
          else:
            if self.batchnorm:
              pass
            out,cache_ = Conv_ReLU.forward(x,self.params["W" + str(ly)],self.params["b" + str(ly)],conv_param=conv_param)
            h_get.append(out)
            cache.append(cache_)
            x = h_get[ly - 1]
        out,cache_ = Linear.forward(x,self.params["W" + str(ly + 1)],self.params["b" + str(ly + 1)])
        h_get.append(out)
        cache.append(cache_)
        scores = h_get[ly]
        #####################################################
        ...
        for j in range(1,self.num_layers + 1):
          loss += self.reg*torch.sum(self.params["W"+str(j)]*self.params["W" + str(j)])
        dh = ds
        dx,grads["W" + str(self.num_layers)],grads["b" + str(self.num_layers)] = Linear.backward(dh,cache[self.num_layers - 1])
        grads["W" + str(self.num_layers)] += 2 * self.reg *self.params["W" + str(self.num_layers)]
        dh = dx
        for ly in range(self.num_layers-1,0,-1):
          if ly - 1 in self.max_pools:
            dx,grads["W" + str(ly)],grads["b" + str(ly)] = Conv_ReLU_Pool.backward(dh,cache=cache[ly - 1])
            grads["W" + str(ly)] += 2 * self.reg *self.params["W" + str(ly)]
            dh = dx
            if self.batchnorm:
              pass
              #h_get[ly - 1],cache[ly -1] = Conv_BatchNorm_ReLU_Pool.forward()
            ...
            if self.batchnorm:
              pass
            dx,grads["W" + str(ly)],grads["b" + str(ly)] = Conv_ReLU.backward(dh,cache=cache[ly - 1])
            grads["W" + str(ly)] += 2 * self.reg *self.params["W" + str(ly)]
            dh = dx
        #############################################################
        #                       END OF YOUR CODE                    #
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data=data_dict,
                  num_epochs = num_epoch, batch_size=128,
                  update_rule=adam,
                  optim_config={
                    'learning_rate': learning_rate,
                  },
                  print_every=3000, device='cuda')
    solver.train()
    #########################################################
    #                  END OF YOUR CODE                     #
    #########################################################
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        std = pow(gain,0.5) / pow(Din*K*K,0.5)
        weight = std * torch.randn((Din,Dout,K,K),device=device,dtype=dtype)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            ##################################################################
            # Replace "pass" statement with your code
            m = torch.nn.BatchNorm1d(x.shape[1],dtype=x.dtype,device=x.device,eps=eps)
            ...
            ################################################################
            # Replace "pass" statement with your code
            x_centerd = x - running_mean
            x_norm = x_centerd / torch.sqrt(running_var + eps)
            out = gamma*x_norm + beta
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
    ...
    def backward_alt(dout, cache):
        ...
        dx_norm = dout * gamma
        dx = (1. / N)/std * (N*dx_norm - torch.sum(dx_norm, dim=0)
		            - x_norm*torch.sum(dx_norm*x_norm, axis=0))
