...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_copy = x_train.clone().cuda().view(num_train,-1)
    x_test_copy = x_test.clone().cuda().view(num_test,-1)
    cuda_dists = torch.zeros(num_train,num_test,device="cuda",dtype = torch.float64)
    for i in range(num_train):
        for j in range(num_test):
            cuda_dists[i, j] = torch.sum((x_train_copy[i] - x_test_copy[j]) ** 2,dtype=torch.float64)
            #cuda_dists[i, j] = torch.pow(torch.sum((x_train_copy[i] - x_test_copy[j]) ** 2,dtype=torch.float64),0.5)
    dists = cuda_dists.cpu()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_copy = x_test.clone().cuda().view(num_test,-1)
    cuda_dists = torch.zeros(num_train,num_test,device="cuda",dtype=torch.float64)
    id_test = torch.arange(x_test_copy.shape[0])
    id_train = torch.arange(x_train_copy.shape[0])
    for i in id_train:
        #cuda_dists[i,id_test] = torch.pow(torch.sum((x_test_copy - x_train_copy[i]) ** 2,dim=1,dtype=torch.float64))
        cuda_dists[i,id_test] = torch.sum((x_test_copy - x_train_copy[i]) ** 2,dim=1,dtype=torch.float64)
    dists = cuda_dists.cpu()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_copy = x_train.clone().cuda().view(num_train,-1)
    x_test_copy = x_test.clone().cuda().view(num_test,-1)
    x_train_norm = (x_train_copy ** 2).sum(dim=1, keepdim=True)
    x_test_norm = (x_test_copy ** 2).sum(dim=1, keepdim=True)
    dists = (x_train_norm + x_test_norm.T - 2 * torch.matmul(x_train_copy,x_test_copy.T)).cpu()
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_folds = x_train.clone().chunk(num_folds)
    y_train_folds = y_train.clone().chunk(num_folds)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for i in k_choices:
        ...
        for j in range(num_folds):
            new_x_train_folds = x_train_folds[:j] + x_train_folds[j + 1:num_folds]
            new_y_train_folds = y_train_folds[:j] + y_train_folds[j + 1:num_folds]
            classifier = KnnClassifier(torch.cat(new_x_train_folds), torch.cat(new_y_train_folds))
            folds_accuracy.append(classifier.check_accuracy(x_test=x_train_folds[j],y_test=y_train_folds[j],k=i))
        k_to_accuracies[i] = folds_accuracy.copy()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    accs = torch.tensor(numpy.array(list(k_to_accuracies.values())))
    accs_row_max,index_col = torch.max(accs,dim=1)
    accs_max,index_col_row = accs_row_max.max(dim=0)
    best_k = list(k_to_accuracies)[index_col_row]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
