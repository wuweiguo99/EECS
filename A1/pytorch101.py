...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    startforten = (start // 10) * 10
    stopforten = (stop // 10) * 10
    if startforten >= stopforten:
        x = torch.tensor([],dtype=torch.float64)
        return x
    x = torch.arange(startforten + 10, stopforten + 10,10,dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idy = torch.arange(x.shape[0])
    y = x[reversed(idy)].clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y0 = x.view(2,3,4)
    y = y0.transpose(0,1).reshape(3,8).clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    col_min_num,col_min_idxs = y.min(dim=1)
    col_idxs = torch.arange(x.shape[0])
    y[col_idxs,col_min_idxs] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    N = x.shape[1]
    y = x.clone()
    col_sum_y = y.sum(dim=0)
    col_sum_y /= N
    std_y = x.clone()
    print(col_sum_y)
    std_y -= col_sum_y
    std_y = torch.pow(std_y,2)
    std_relly_y = std_y.sum(dim=0)
    std_relly_y /= (N - 1)
    std_relly_y = torch.pow(std_relly_y,0.5)
    y = (y - col_sum_y) / std_relly_y
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
